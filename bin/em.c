...
void cpu(uint pc, uint sp) {
  uint a, b, c, ssp, usp, t, p, v, u, delta, cycle, xcycle, timer, timeout,
    fpc, tpc, xsp, tsp, fsp;
  double f, g;
  int ir, *xpc, kbchar;
  char ch;
...
fatal:
  dprintf(2,
	  "processor halted! cycle = %u pc = %08x ir = %08x sp = %08x a = %d b = %d c = %d trap = %u\n",
...
int main(int argc, char *argv[]) {
  ...
  if (fs) {
    ...
    if ((i =
	 read(f, (void *) (mem + memsz - FS_SZ), st.st_size)) != st.st_size) {
