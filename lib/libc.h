...
sleep() {
  asm(LL, 8);
  asm(TRAP, S_sleep);
}				// XXX is this seconds? should it be?
uptime() {
  asm(TRAP, S_uptime);
}
lseek() {
...
// XXX wrong for now!
int sscanf(char *s, char *f, int *a) {
  if (strcmp(f, "%i"))
    *(double *) a = 2.1;
  else
    *a = 1;
...
int vsprintf(char *s, char *f, va_list v) {
  ...
  while (c = *f++) {
	...
	for (fmax = 0; '0' <= *f && *f <= '9'; fmax = fmax * 10 + *f++ - '0');
	prec = fmax;
      }
    }
    if (*f == 'l')
...
//            while (i >= 0 || d > 0.0) { if (!i-- && prec) *--p = '.'; *--p = '0' + ((d > 1.0e15) ? 0 : ((int)fmod(d+0.5,10.0))); d = floor(d * 0.1); }  XXX
	while (i >= 0 || d > 0.0) {
	  ...
	  *--p =
	    '0' +
	    ((d > 1000000000000000.0) ? 0 : ((int) fmod(d + 0.5, 10.0)));
	  d = floor(d * 0.1);
	}
	i = (b + BUFSIZ - 1) - p;
	break;
...
int sprintf(char *s, char *f, ...) {
  va_list v;
  va_start(v, f);
  return vsprintf(s, f, v);
}
int printf(char *f, ...) {
  char s[BUFSIZ];
  va_list v;
  va_start(v, f);
  return vwrite(1, s, vsprintf(s, f, v));
}
int vprintf(char *f, va_list v) {
  char s[BUFSIZ];
  return vwrite(1, s, vsprintf(s, f, v));
}
int dprintf(int d, char *f, ...) {
  char s[BUFSIZ];
  va_list v;
  va_start(v, f);
  return vwrite(d, s, vsprintf(s, f, v));
}
int vdprintf(int d, char *f, va_list v) {
  char s[BUFSIZ];
  return vwrite(d, s, vsprintf(s, f, v));
}
...
int stat(char *n, struct stat *s) {
  int f, r;
  if ((f = open(n, O_RDONLY)) < 0)
    return -1;
  r = fstat(f, s);
  close(f);
  return r;
